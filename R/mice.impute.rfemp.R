#' Random Forest EMPowered IMPutation
#'
#' @description
#' \code{RfEmpImp} imputation methods, adapter for \code{mice} samplers.
#' These functions can be called by the \code{mice} sampler function.
#'
...
#'
#' \code{mice.impute.rfemp.cont} is for continuous variables, performing
#' imputation based on the empirical distribution of out-of-bag
#' prediction errors of random forests.
#'
#' \code{mice.impute.rfemp.cate} is for mixed categorical variables,
...
mice.impute.rfemp <- function(
    ...
    if (is.numeric(y)) {
        if (is.null(num.trees.cont)) num.trees.cont <- num.trees
        rfImpVec <- mice.impute.rfemp.cont(
            y = y,
            ry = ry,
            x = x,
            wy = wy,
        ...
        if (is.null(num.trees.cate)) num.trees.cate <- num.trees
        rfImpVec <- mice.impute.rfemp.cate(
            y = y,
            ry = ry,
            x = x,
            wy = wy,
